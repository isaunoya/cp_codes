...
public:
  ...
  BigInteger &operator=(int num) { return *this = BigInteger(num); }
  BigInteger &operator=(long long num) { return *this = BigInteger(num); }
  BigInteger &operator=(const string &str) { return *this = BigInteger(str); }
  BigInteger &operator=(const char *str) { return *this = BigInteger(str); }

  // relational operators
  bool operator<(const BigInteger &obj) const { return cmp(obj) < 0; }
  bool operator>(const BigInteger &obj) const { return cmp(obj) > 0; }
  bool operator<=(const BigInteger &obj) const { return cmp(obj) <= 0; }
  bool operator>=(const BigInteger &obj) const { return cmp(obj) >= 0; }
  bool operator==(const BigInteger &obj) const { return cmp(obj) == 0; }
  ...
  BigInteger operator-() const { return BigInteger(-sign_, val_); }
...
BigInteger BigInteger::operator+(const BigInteger &obj) const {
  if (sign_ * obj.sign_ == 1) {
    BigInteger temp;
    return cmp(obj, without_sign) >= 0 ? (temp = *this).add(obj)
                                       : (temp = obj).add(*this);
...
BigInteger BigInteger::operator-(const BigInteger &obj) const {
  if (sign_ * obj.sign_ == 1) {
    BigInteger temp;
    return cmp(obj, without_sign) >= 0 ? (temp = *this).sub(obj)
                                       : (temp = -obj).sub(*this);
  } else if (sign_ * obj.sign_ == -1)
    return *this + -obj;
  else
...
inline BigInteger BigInteger::operator/(const BigInteger &obj) const {
  ...
  return cmp(obj, without_sign) < 0 || (temp.sign_ = sign_ * obj.sign_) == 0
             ? temp
             : temp.div(mod, obj);
}
...
inline BigInteger BigInteger::operator%(const BigInteger &obj) const {
  BigInteger temp, mod = *this;
  return cmp(obj, without_sign) < 0 || (temp.sign_ = sign_) == 0
             ? mod
             : temp.div(mod, obj, remainder);
}
...
int main() {
  ...
  for (int op = 0; op <= 1; op++)
    if (u % C[op] == v % C[op])
      a[op] = b[op] = gcd(nu, query(findk(u, op)));
  if (a[0] == 0 && b[1] == 0) {
    long long k = CRT({(int)(u % C[0]), (int)(v % C[1])}, {C[0], C[1]});
    while (k % C[2] == u % C[2] || k % C[2] == v % C[2] ||
           k % C[1] == u % C[1] || k % C[0] == v % C[0])
      k += (long long)C[0] * C[1];
  ...
  for (int op = 0; op <= 1; op++) {
    if (a[op] == 0)
      a[op] = gcd(nu, query(findk(u, op)));
    if (b[op] == 0)
      b[op] = gcd(nv, query(findk(v, op)));
  }
